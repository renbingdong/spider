...
class CrawlPageModel extends BaseModel {
    ...
    public function isExist($url_hash_code) {
        $sql = "select 1 from {$table_name} where url_hash_code = '{$url_hash_code}'";
    ...
    public function insert($url, $url_hash_code, $summary_context, $file_path) {
        $sql = "insert into {$table_name} (`url`, `url_hash_code`, `page_summary_context`, `page_file_path`, `c_t`) values ";
        $now = time();
